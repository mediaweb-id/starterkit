...
const edit = (node) => {
    ...
    showPopup.value = 'InputCreateField';
    isEdit.value = true;
    save();
}
const close = () => {
...
const choosedComponent = (comp) => {
    useComponent.value = {id:'section-'+getId(), data:null, type: 'InputCreateField', title:'Create Section', show:true };
...
<template>
  <div @mouseleave="save">
        ...
        <div class="flex w-full mb-2 border-b">
            ...
            <div class="flex w-full mr-4 items-center">
                <div class="w-full m-2 text-sm">
                    <strong class="font-semibold">{{ node.data.title }}</strong> Section
                </div>
            </div>
    ...
    <div class="flex">
        <AOutlineButton @click="choosedComponent('InputCreateField')"  type="button" class="justify-center "> <i class="fas text-white-400 mr-2 text-sm fa-plus"></i> Add New Section</AOutlineButton>
    </div>

    <div class="fixed left-0 top-0 w-full h-full  z-[1000]" v-if="showPopup == 'InputCreateField'">
            ...
            <div class="flex flex-wrap w-full p-5">
                <div class="pt-5 px-5 bg-white max-w-[700px] w-[90%] text-center mx-auto rounded-md relative" >
                    <div class="border-b  pb-3  mb-5 flex gap-5 ">
                        <h3 class="font-bold">{{ useComponent.title }}</h3>
                    </div>
                    <InputCreateField v-model="useComponent.data"></InputCreateField>
                    <div class="border-t mt-5 py-5 flex gap-5 justify-end">
                        <AOutlineButton @click="close"  class="justify-center cursor-pointer"> <i class="fas text-white-400 mr-2 text-sm fa-xmark"></i> Close</AOutlineButton>
                        <APrimaryButton @click="savedata"  class="justify-center cursor-pointer"> <i class="fas text-white-400 mr-2 text-sm fa-check"></i> Save</APrimaryButton>
                    </div>
                </div>
            </div>
        </div>
